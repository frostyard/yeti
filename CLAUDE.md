...
- **`github-app.ts`** — Optional GitHub App authentication. Signs JWTs (RS256 via Node.js `crypto`), manages installation token lifecycle with 5-min pre-expiry refresh and in-flight dedup. Sets `process.env.GH_TOKEN` so all `gh`/`git` subprocesses inherit the App identity automatically. Graceful fallback: if no App config, everything works via personal `gh` CLI auth.
- **`config.ts`** — Configuration priority: env vars > `~/.yeti/config.json` > defaults. Uses ESM `export let` for live reloads without restart. Exports `LABELS`, `INTERVALS`, `SCHEDULES`, `ALLOWED_REPOS`, `ENABLED_JOBS`, `JOB_AI`, `QUEUE_SCAN_INTERVAL_MS`, `GITHUB_APP_ID`, `GITHUB_APP_INSTALLATION_ID`, `GITHUB_APP_PRIVATE_KEY_PATH`, etc. Per-job AI backend/model overrides via `jobAi` config map. GitHub App fields are immutable (require restart).
...
- **`server.ts`** — HTTP dashboard with job status, jobs detail page (`/jobs`), work queue, log viewer, config editor, and OAuth routes (`/auth/github`, `/auth/callback`, `/auth/logout`). Auth is enabled when `authToken` is set or OAuth is configured (either or both). `createServer(scheduler, allJobs)` accepts job metadata for the Jobs page.
