...
- **`server.ts`** — HTTP dashboard with job status, jobs detail page (`/jobs`), work queue, log viewer, config editor. Token-based auth when `authToken` is set. `createServer(scheduler, allJobs)` accepts job metadata for the Jobs page.
