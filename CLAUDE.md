...
- **`config.ts`** — Configuration priority: env vars > `~/.yeti/config.json` > defaults. Uses ESM `export let` for live reloads without restart. Exports `LABELS`, `INTERVALS`, `SCHEDULES`, `ALLOWED_REPOS`, `ENABLED_JOBS`, `JOB_AI`, `QUEUE_SCAN_INTERVAL_MS`, etc. Per-job AI backend/model overrides via `jobAi` config map.
