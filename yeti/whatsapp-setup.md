...
- A working Yeti installation (systemd service running, `gh` and `claude`
  authenticated)
...
- Terminal access (SSH or local) to the Yeti host for the initial QR-code
  pairing step
...
| **Yeti compatibility** | Fully compatible, no changes needed | Not used by Yeti |
| **Cost** | Free | Pay-per-conversation pricing from Meta |

**Yeti uses the Baileys library, which connects via the WhatsApp Web linked
...
5. Use this number for the Yeti QR-code pairing step below
...
Add the numbers to `~/.yeti/config.json`:
...
Or set environment variables in `~/.yeti/env`:
...
Add the key to `~/.yeti/config.json`:
...
3. The logs should show `[whatsapp] Connected` once pairing completes.
   Credentials are saved to `~/.yeti/whatsapp-auth/` — subsequent restarts
...
Check the Yeti status endpoint:

```bash
curl -s http://localhost:9384/status | jq .whatsapp
```
...
The web dashboard at `http://localhost:9384/` also shows WhatsApp connection
...
- [ ] `~/.yeti/config.json` has `whatsappEnabled: true` and at least one
      number in `whatsappAllowedNumbers`
- [ ] Yeti service is running: `sudo systemctl status yeti`
...
1. Edit `~/.yeti/config.json` and modify the `whatsappAllowedNumbers` array
   (or update `WHATSAPP_ALLOWED_NUMBERS` in `~/.yeti/env`)
2. Restart the service: `sudo systemctl restart yeti`
...
- Check the journal for errors: `sudo journalctl -u yeti -e`
...
- Check that the phone running WhatsApp is still powered on and connected to
  the internet
- Check `sudo journalctl -u yeti -e` for disconnect/reconnect messages
...
- Auth credentials are stored locally in `~/.yeti/whatsapp-auth/`. Protect
  this directory — anyone with access to these files can impersonate the linked
  device.
- Messages are not stored in the Yeti database. They are processed in memory
