...
function buildNewPlanPrompt(fullName: string, issue: gh.Issue, comments: gh.IssueComment[]): string {
  return [
    ...
    `If \`yeti/OVERVIEW.md\` exists in the repository, read it first (and any linked documents that seem relevant to the issue) for context about the codebase architecture and patterns.`,
    ``,
    `Before producing a plan, evaluate whether the issue provides enough detail`,
    `to create a confident implementation plan. Consider:`,
    ...
    ``,
    `If the issue is underspecified, DO NOT guess. Instead, output a section titled`,
    `"### Clarifying Questions" listing specific questions that would need answers`,
    `before a reliable plan can be written. Be concrete — reference the parts of the`,
    `issue that are ambiguous and suggest options where possible.`,
    ``,
    `After listing your clarifying questions, instruct the user to respond to them`,
    `as a comment on the GitHub issue so that the next refinement cycle can`,
    `incorporate their answers and produce a complete plan.`,
    ``,
    `Only produce the implementation plan for aspects that are sufficiently clear.`,
    `If nothing is clear enough to plan, output only the clarifying questions and`,
